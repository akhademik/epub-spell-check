...
-   **Kiểm tra chính tả toàn diện**: Phát hiện các từ không có trong từ điển Viet74K.
...
-   **Bỏ qua tên riêng/từ nước ngoài**: Tùy chọn bỏ qua các từ có vẻ là tên riêng hoặc từ nước ngoài được tải từ danh sách `names.txt`.
...
1.  **Mở ứng dụng**: Mở tệp `index.htm` trong trình duyệt web của bạn (ví dụ: Chrome, Firefox, Edge).
2.  **Tải tệp EPUB**:
    *   Kéo và thả tệp `.epub` của bạn vào khu vực "Tải lên sách EPUB".
...
5.  **Quản lý danh sách bỏ qua (Whitelist)**:
    ...
    *   Tùy chọn "Bỏ qua tên riêng/từ nước ngoài" (checkbox) sẽ thêm các từ từ `names.txt` vào danh sách bỏ qua, hữu ích cho các sách có nhiều tên riêng hoặc từ ngữ tiếng Anh/tiếng nước ngoài.
6.  **Kiểm tra tệp khác**: Nhấp vào nút "Kiểm tra tệp khác" ở góc trên bên phải để tải một tệp EPUB mới.

## Yêu cầu

-   Trình duyệt web hiện đại (Chrome, Firefox, Edge, Safari).
-   Tệp sách định dạng `.epub`.

## Phát triển & Tùy chỉnh

Mã nguồn được viết bằng HTML, CSS (Tailwind CSS) và JavaScript thuần, dễ dàng tùy chỉnh.

-   **Từ điển**: Tệp `vn-dict.txt` chứa danh sách các từ tiếng Việt được sử dụng để kiểm tra.
-   **Danh sách bỏ qua hệ thống**: Tệp `whitelist.txt` và `names.txt` (cho tên riêng/từ nước ngoài) cung cấp các từ được bỏ qua mặc định.
-   **CSS**: Sử dụng Tailwind CSS để tạo kiểu.
