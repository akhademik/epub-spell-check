import "./style.css"
import { EPUB_FILE_EXTENSION, SETTINGS_FILTER_DEBOUNCE_MS } from "./constants"
...
import { type CheckSettings, groupErrors } from "./utils/analyzer"
...
function selectNextError(wordToIgnoreId: string, originalIndex: number) {
  ...
  if (currentFilteredErrors.length > 0) {
    ...
    if (reFoundIndex !== -1) {
      ...
      targetIndex = Math.min(originalIndex, currentFilteredErrors.length - 1)
...
function performExport(type: "vctve" | "normal") {
  ...
  if (type === "vctve") {
    ...
    content = currentFilteredErrors.map((_g: ErrorGroup) => _g.word).join("\n")
  }
...
function selectGroup(group: ErrorGroup, element: HTMLElement) {
  $appState.get().selectedErrorElement?.classList.remove("selected-error-item")
...
function navigateInstance(direction: "prev" | "next") {
  ...
  renderContextView(UI, currentGroup, nextInstanceIndex, dictionaries)
  updateNavButtons()
}
