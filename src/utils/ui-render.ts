...
function getErrorHighlights(type: string): {
  ...
  if (!type || !Object.hasOwn(FIXED_ERROR_COLORS, type)) {
...
export function renderErrorList(ui: UIElements, groups: ErrorGroup[]) {
  ...
  requestAnimationFrame(() => {
    ...
    ui.virtualizedErrorList = new SimpleVirtualScroll(
      // biome-ignore lint/style/noNonNullAssertion: ui.errorList is guaranteed to be present after initialization or the app would crash.
      ui.errorList!,
      groups,
      rowHeight,
      (group: ErrorGroup) => {
        ...
        element.className =
          "flex items-stretch w-full mb-1 transition-all border border-transparent rounded-lg hover:bg-slate-800 group bg-slate-900 hover:border-slate-700"
        ...
        selectBtn.className =
          "flex items-center justify-between flex-grow px-4 py-3 text-left border-r rounded-l-lg select-btn focus:outline-none border-slate-800/50 group-hover:border-slate-700"
        ...
        wordSpan.className =
          "font-serif text-lg font-bold truncate text-slate-200 error-word"
        wordSpan.textContent = group.word

        const countSpan = document.createElement("span")
        countSpan.className =
          "bg-slate-800 text-slate-400 border border-slate-700 text-[10px] font-bold px-2 py-0.5 rounded-full error-count flex-shrink-0"
        ...
        reasonDiv.className =
          "mt-1 text-xs truncate text-slate-500 error-reason"
        ...
        ignoreBtn.className =
          "flex items-center justify-center px-3 transition-colors rounded-r-lg ignore-btn text-slate-600 hover:text-green-400 hover:bg-slate-800/50"
