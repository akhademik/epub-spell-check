<script lang="ts">
  ...
  function handleSaveToken() {
    ...
    loadAuth()
    appState.showToast("Đã lưu token quản trị vào trình duyệt", "success")
  }
    ...
    <div class="flex items-center gap-2 text-xs">
      {#if authStatus.authType === "cloudflare-access"}
        <div class="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-emerald-500/10 border border-emerald-500/30 text-emerald-400 font-medium">
          ...
          <span>Cloudflare Access: <strong>{authStatus.email ?? 'Authenticated'}</strong></span>
        </div>
      ...
      {:else}
        <div class="flex items-center gap-2">
          <input
            ...
            placeholder="Nhập ADMIN_TOKEN dự phòng"
            class="px-3 py-1.5 text-xs bg-slate-950 border border-slate-700 rounded-xl text-slate-200 focus:outline-none focus:border-blue-500 font-mono"
          />
          <button
            type="button"
            onclick={handleSaveToken}
            class="px-3 py-1.5 text-xs font-medium text-white bg-blue-600 hover:bg-blue-500 rounded-xl transition-colors"
          >
            Lưu
          </button>
        </div>
  ...
  <div class="grid grid-cols-1 lg:grid-cols-12 gap-6">
    ...
    <div class="lg:col-span-7 flex flex-col space-y-4">
      ...
      <div class="flex-1 flex flex-col rounded-2xl bg-slate-900 border border-slate-800 shadow-xl overflow-hidden min-h-[500px]">
        ...
        <!-- Words List (Virtualized / Scrollable) -->
