...
*   **Duplicated Whitelist Parsing Logic**: The logic for splitting and trimming whitelist input (e.g., `currentWhitelistRaw.split(/[,;"]+/).map((t: any) => t.trim()).filter(Boolean)`) is repeated in `quickIgnore()`, `quickIgnoreWordFromList()`, `exportWhitelist()`, and `filterAndRenderErrors()`.
    *   **Improvement**: Extract this into a dedicated helper function (e.g., `parseWhitelistInput(inputString: string): Set<string>`) to improve reusability and maintainability, and reduce the need for `// eslint-disable-next-line @typescript-eslint/no-explicit-any`. This helper function could reside in a new `utils/whitelist.ts` module.
